...
const Menus = () => {
  ...
  return full ? (
    <div className={`${styles.menus} ${btn_state ? styles.menus_hot : ""}`}>
      ...
      <div className={styles.icons}>
        ...
        <div
          className={`${styles.btn_icon} ${btn_state ? styles.hot : ""}`}
          onClick={btn_click}
        />
      </div>
      <ul className={styles.menus_ul}>
        {menu_list.map((u) => {
          return (
            <li
              key={u.path}
              onClick={() => to_path(u.path)}
              className={`${styles.menus_li} ${
                path_name === u.path ? styles.hot : ""
              }`}
            >
              {u.name}
            </li>
        ...
        {menu_list.length === 0 ? (
          <li style={{ textAlign: "center", color: "#ccc", fontSize: 14 }}>
            暂无更多
          </li>
        ) : (
          ""
